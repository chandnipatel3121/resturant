...
const ParallaxHero = () => {
  ...
  const clipPath = useTransform(morphProgress, (v) => {
    ...
    const startRadius = Math.sqrt(vw * vw + vh * vh) * (isMobileLocal ? 0.8 : 0.6)
    const finalRadius = isMobileLocal ? vh * 0.18 : vh * 0.22
  ...
  const forkX = useTransform(morphProgress, (v) => {
    ...
    const offset = isMobileLocal ? 18 : 22
    if (v <= 0.5) return `calc(-50vw + ${(v / 0.5) * (isMobileLocal ? 15 : 10)}vw - 100% - 2vh)`
    const p = (v - 0.5) / 0.5
    return `calc(-${isMobileLocal ? 35 : 40}vw * ${1 - p} - ${offset}vh * ${p} - 100% - 1vh)`
  })
  const spoonX = useTransform(morphProgress, (v) => {
    const isMobileLocal = window.innerWidth < 768
    const offset = isMobileLocal ? 18 : 22
    if (v <= 0.5) return `calc(50vw - ${(v / 0.5) * (isMobileLocal ? 15 : 10)}vw + 2vh)`
    const p = (v - 0.5) / 0.5
    return `calc(${isMobileLocal ? 35 : 40}vw * ${1 - p} + ${offset}vh * ${p} + 1vh)`
  })
        ...
        <motion.div style={{ opacity: s2Opacity }} className="absolute inset-0 z-10">
          ...
          >
            ...
            <div className="hero-btn-group">
              ...
              <button onClick={() => navigate("/menu")} className="btn-primary">Take away</button>
            </div>
          </motion.div>
        </motion.div>
